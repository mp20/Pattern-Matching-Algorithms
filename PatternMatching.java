...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
                                           ...
                                           CharacterComparator comparator) {
        if (pattern == null || pattern.length() == 0) {
            throw new IllegalArgumentException("patten is null or zero");
        }
        if (text == null || comparator == null) {
            throw new IllegalArgumentException("text or comparator is(are) null");
        }
        List<Integer> list = new ArrayList<>();
    ...
    public static Map<Character, Integer> buildLastTable(CharSequence pattern) {
        if (pattern == null) {
            throw new IllegalArgumentException("can not have a null pattern");
        }
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        if (pHash == nHash && check(pattern, text.subSequence(0, pattern.length()), comparator)) {
            list.add(0);
        }
        for (int i = 1; i < text.length() - pattern.length() + 1; i++) {
            nHash = (nHash - (text.charAt(i - 1) * power(BASE, pattern.length() - 1)))
                    * BASE + text.charAt(i + pattern.length() - 1);
            if (nHash == pHash && check(pattern, text.subSequence(i, i + pattern.length()), comparator)) {
                list.add(i);
            }
        }
        return list;
    ...
    /**
     * calculates the hashing portion
     *
    ...
    private static int hash(CharSequence text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            hash += text.charAt(i) * power(BASE, text.length() - 1 - i);
        }
        return hash;
    }
     ...
     *
     * @param a wil be raised to b
     * @param b is the exponent of a
     * @return a^b
     */
    ...
    /**
     * Checks if texts and patterns match
     *
     * @param pattern desired text
     * @param text the entire string which you search the pattern in for
     * @param comparator used to compare values
     * @return true if pattern and text match, false otherwise
     */
